---
description: Créer une tâche destinée à être exécuté plus tard
...
IF $ARGUMENTS is empty or vague (user just ran `/task:create` without details):
...
<action>
If user chooses #1, invoke via SlashCommand tool: `/task:run 005`
</action>
</single_prompt_scenario>
...
<actions>
If user chooses #1, invoke via SlashCommand tool: `/task:run 005 006 007 --parallel`
If user chooses #2, invoke via SlashCommand tool: `/task:run 005 006 007 --sequential`
</actions>
</parallel_scenario>
...
<actions>
If user chooses #1, invoke via SlashCommand tool: `/task:run 005 006 007 --sequential`
If user chooses #2, invoke via SlashCommand tool: `/task:run 005`
</actions>
</sequential_scenario>
...
- Use the SlashCommand tool to invoke /task:run when user makes their choice
</meta_instructions>
